...
angleRep = linspace(10,80);
deform = zeros(6,length(angleRep));
minus = zeros(6,6,length(angleRep));
for ite=1:length(angleRep)
    angl = angleRep(ite);
    ...
    A = sum(Q_.*reshape((coteZ(:,1)-coteZ(:,2)),1,1,nbstrat),3)/1; %MPa.m
    B = sum(Q_.*reshape((coteZ(:,1).^2-coteZ(:,2).^2),1,1,nbstrat),3)./2; %MPa.m²
    D = sum(Q_.*reshape((coteZ(:,1).^3-coteZ(:,2).^3),1,1,nbstrat),3)./3; %MPa.m^3
    E = sum(Q_.*reshape((coteZ(:,1).^4-coteZ(:,2).^4),1,1,nbstrat),3)./4; %MPa.m^3
...
figure
plot(angleRep,squeeze(minus(4,4,:)).*1000)
hold on
plot(angleRep,squeeze(minus(6,6,:)).*1000)
xlabel('angle stratifié','interpreter','latex')
ylabel('complaisance $kN^{-1}.m^{-1}$','interpreter','latex')
legend('d_{11}','d_{66}')
grid on
