...
angleInt = linspace(10,80,50);
angleExt = linspace(10,80,50);
...
for j=1:length(angleInt)
    for k=1:length(angleExt)
        ...
        e_ply = 1;
        e_coeur=10*0.001;%demi épaisseur du coeur vide
        strat = [
            1 e_ply -angl1;
            1 e_ply angl2;
            ];
    ...
    htot = sum(strat(:,2));
    R = (e_coeur+sum(strat(find(strat(:,1)),2))./2);
    ...
    for i=1:nbstrat
        ...
        A = sum(Q_.*reshape((coteZ(:,1)-coteZ(:,2)),1,1,nbstrat),3)/1; %Pa.m
        B = sum(Q_.*reshape((coteZ(:,1).^2-coteZ(:,2).^2),1,1,nbstrat),3)./2; %Pa.m²
        D = sum(Q_.*reshape((coteZ(:,1).^3-coteZ(:,2).^3),1,1,nbstrat),3)./3; %Pa.m^3
        E = sum(Q_.*reshape((coteZ(:,1).^4-coteZ(:,2).^4),1,1,nbstrat),3)./4; %Pa.m^3
        ...
        MAJ = [A_ B_;B_ D_];
        minus(:,:,ite) = inv(MAJ).*10^6;
        prod1(j,k)=minus(4,4,ite)*minus(6,6,ite);
        prod2(j,k)=minus(4,4,ite)*(minus(6,6,ite)^0.5);
        ite = ite + 1;
    end
end
...
figure
surf(angleInt,angleExt,prod1)
hold on
scatter3(idx_min_prod1,idy_min_prod1,min_prod1)
hold off
xlabel('$\theta_{int}$','interpreter','latex')
ylabel('$\theta_{ext}$','interpreter','latex')
zlabel('$\overline{d_{11}}*\overline{d_{66}}','interpreter','latex')

figure
surf(angleInt,angleExt,prod2)
hold on
scatter3(idx_min_prod2,idy_min_prod2,min_prod2)
hold off
xlabel('$\theta_{int}$','interpreter','latex')
ylabel('$\theta_{ext}$','interpreter','latex')
zlabel('$\overline{d_{11}}*\sqrt{\overline{d_{66}}}$','interpreter','latex')
