...
e = linspace(0.1,2);
deform = zeros(6,length(e));
minus = zeros(6,6,length(e));
d11 = zeros(101,1);
d66=d11;
for ite=1:length(e)+1
        %strat: code(=1,2,3ou4 si mat et 0 si rien) epaisseur (mm) angle (°)
        if ite~=length(e)+1
        strat = [
            4 e(ite) 15;
            ];
        e_coeur = 12-e(ite); %demi épaisseur du coeur vide
        else
            strat = [
                1 1 15.71;
                1 1 -15.71;
        ...
        MAJ = [A_ B_;B_ D_];
        minus(:,:,ite) = inv(MAJ);
    %d11(ite)=squeeze(minus(4,4,ite));
    %d66(ite)=squeeze(minus(6,6,ite));
    d11(ite,1)=1/MAJ(4,4);
    d66(ite,1)=1/MAJ(6,6);
end

figure
plot(12-e,d11(1:end-1,1).*1000)
hold on
plot(12-e,d11(end,1).*ones(length(e)).*1000)
xlabel('R_{int}','interpreter','latex')
ylabel('$\overline{d_{11}}$ $kN^{-1}.m^{-1}$','interpreter','latex')
legend('acier','carbon [15.71/-15.71]')
hold off

figure
plot(12-e,d66(1:end-1,1).*1000)
hold on
plot(12-e,d66(end,1).*ones(length(e)).*1000)
xlabel('R_{int}','interpreter','latex')
ylabel('$\overline{d_{66}}$ $kN^{-1}.m^{-1}$','interpreter','latex')
legend('acier','carbon [15.71/-15.71]')
hold off
